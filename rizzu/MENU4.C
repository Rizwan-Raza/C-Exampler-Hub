...
char aa[50][50],som[8]="Salary",eos[10]="Employee",filename[32];
float *sal;
FILE *ptr;
void display_data();
...
void new_db()
{
    ...
    do
    {
        ...
        {
            gotoxy(4,7);
            printf("Sorry you can't create file with this name");
        }
    }
    while(!ptr);
    store_data(ptr);
    fclose(ptr);
    gotoxy(52,6);
    printf("The Database has been stored in %s file.");
    gotoxy(52,7);
    printf("You can read and modify it using our another features");
    getch();
}
void open_db()
{
    ...
    {
        gotoxy(4,7);
        printf("Sorry we can't find file %s in current directory",filename);
    }
    }
	while(!ptr);
	retrieve_data(ptr);
...
void store_data(FILE *output)
{
	for(i=0;i<n;i++)
	{
		...
		fprintf(output,"%-25s",aa[i]);
		fseek(output,2,SEEK_CUR);
		fprintf(output,"%-2d",x[i]);
		fseek(output,2,SEEK_CUR);
		fprintf(output,"%-3.2f",sal[i]);
		fprintf(output,"\n");
	}
	fclose(output);
}
